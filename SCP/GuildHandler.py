...
class GuildHandler(commands.Cog):
    ...
    async def settings(self, ctx, command=None, value=None):
        ...
        if not command:
            embed = discord.Embed(title = 'Server Settings', description = 'This servers settings.\n use `%ssettings "<command>" <enable|disable>`\n `%sconfiguration` is to alter server settings, eg. `%sconfiguration setprefix <prefix>`.\n [Server Dashboard](%s)'%(DiscordGuild.find_one({"id":ctx.author.guild.id}, {"prefix"})["prefix"],DiscordGuild.find_one({"id":ctx.author.guild.id}, {"prefix"})["prefix"],DiscordGuild.find_one({"id":ctx.author.guild.id}, {"prefix"})["prefix"], Globals.getDashboardURL()))
